...
// --- Compliance ---
export const getSubjects = () => jget("/compliance/saved_subjects").then((d) => d.data || []);
export const saveSubjects = (subjects: any[]) => jpost("/compliance/saved_subjects", subjects);
export const getVisitLogs = () => jget("/compliance/visit_logs").then((d) => d.data || []);
...
export const invoice = (subject?: string) =>
  jget("/dispatch/invoice" + (subject ? `?subject=${encodeURIComponent(subject)}` : ""));
