...
export default function SubjectDetail() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setMsg(e instanceof AuthError ? "Please sign in to view this subject." : String(e));
      }
    })();
  }, [sid]);
  ...
  return (
    <div>
      <p className="muted"><Link href="/subjects">← Subjects</Link></p>
      <h1>{sid}</h1>
      ...
      <div className="card">
        <h3>Compliance</h3>
        {subject ? (
          <p>Status: <span className="badge ok">{subject.status || "—"}</span>
            {subject.med ? <> · Medication: <strong>{subject.med}</strong></> : null}</p>
      ...
      <div className="card">
        ...
        {logs.length ? (
          <table>
            <thead><tr><th>Visit</th><th>Status</th></tr></thead>
            <tbody>{logs.map((l, i) => <tr key={i}><td>{l.visit || "—"}</td><td>{l.status || "—"}</td></tr>)}</tbody>
          </table>
      ...
      <div className="card">
        ...
        {trips.length ? (
          <table>
            <thead><tr><th>Date</th><th>Vehicle</th><th>Miles</th><th>Amount</th></tr></thead>
            <tbody>{trips.map((t) => (
              <tr key={t.id}><td>{t.date}</td><td>{t.type}</td><td>{Number(t.miles).toFixed(1)}</td><td>${Number(t.amount).toFixed(2)}</td></tr>
            ))}</tbody>
          </table>
        ) : <p className="muted">None.</p>}
      </div>
      ...
      {inv && (
        <div className="card">
          ...
          <p>Trips: <strong>{inv.trip_count}</strong> · Miles: <strong>{inv.total_miles}</strong> ·
            {" "}Total due: <strong>${Number(inv.total_due).toFixed(2)}</strong></p>
