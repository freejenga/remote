...
function abbrev(name: string, cycle?: string): string {
  ...
  const wk = n.match(/week\s*(\d+)/);
  if (wk) return 'W' + wk[1];
  const cy = n.match(/cycle\s*(\d+)/);
  if (cy) return 'C' + cy[1];
...
export default function ProtocolPage() {
  ...
  async function run() {
    ...
    try {
      ...
      setMsg(e instanceof AuthError ? 'Please sign in to parse.' : String(e));
    } finally {
      setBusy(false);
    }
  }
  ...
  function exportCsv() {
    ...
    a.href = url;
    a.download = 'ctms_rows.csv';
    a.click();
    URL.revokeObjectURL(url);
  }

  const srcBadge = (s: string) => (
    <span className={'badge ' + (s === 'narrative' ? 'src-narr' : 'src-soa')}>
      {s === 'narrative' ? 'narr' : 'soa'}
    </span>
  );
        ...
        <>
          ...
          {view === 'review' ? (
              ...
              <div className="ws-side">
                ...
                {visits.map((v, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className={'visit-item ' + (i === sel ? 'active' : '')}
                      onClick={() => setSel(i)}
                    >
                      <div className="nm">{v.name}</div>
                      <div className="mt">
                        <span className="badge gray">
                          {abbrev(v.name, v.cycle_group)}
                        </span>
                        <span className="badge gray">
                          {rowsForVisit(v.name).length} acts
                        </span>
              ...
              <div className="card" style={{ margin: 0 }}>
                ...
                <table>
                  ...
                  <tbody>
                    ...
                    {rowsForVisit(visits[sel]?.name).length === 0 && (
                      <tr>
                ...
                >
                  <div className="stat-card">
                    <div className="val">{visits.length}</div>
                ...
                {conflicts.map((c, i) => {
                  ...
                  return (
                    <div key={i} className={'conflict-card ' + (info ? 'info' : '')}>
                      <div
                        className="ch"
                        onClick={() => setOpenConflict(openConflict === id ? null : id)}
                      >
                        <span>{info ? 'ℹ️' : '⚠️'}</span>
                        <div>
            ...
            <div className="card">
                  ...
                  <tbody>
                    {matrix.acts.map((act, ri) => {
                      ...
                      return (
                        <tr
                          key={ri}
                          style={narr ? { background: '#fef3c7' } : undefined}
                        >
                          <td className="proc">
                            {act}
                            {narr && (
                          ...
                          {visits.map((v, ci) => {
                            ...
                            return (
                              <td key={ci} style={{ textAlign: 'center' }}>
                                {cell ? (
                                  cell.condition ? (
                                    <span className="soa-c">X*</span>
                                  ) : (
                                    <span className="soa-x">X</span>
                                  )
                                ) : (
              ...
              >
                <span>
                  <span className="soa-x">X</span> Present
                </span>
                <span>
                  <span className="soa-c">X*</span> Conditional
                </span>
